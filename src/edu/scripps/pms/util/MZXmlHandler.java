...
/**
 * @author Robin Park
 * @version $Id: MZXmlHandler.java,v 1.3 2007/11/26 19:32:16 rpark Exp $
 */
public class MZXmlHandler 
{
	...
	for (i = 0; i < byteArr.length - floatBytes + 8; i += floatBytes)
	{
		...
		pList.addPeak(new Peak((float)mass, (float)intensity));
    ...
    public static NativePeakList decode32ToArr(String input)
    {
	byte[] byteArr = Base64.decode(input);
	...
	gnu.trove.TDoubleArrayList massArr = new gnu.trove.TDoubleArrayList();
	gnu.trove.TDoubleArrayList intArr = new gnu.trove.TDoubleArrayList();
	...
	for (i = 0; i < byteArr.length - floatBytes + 4; i += floatBytes)
	{
		...
		massArr.add( (double)mass );
	    }
	    else
	    {
		intensity = Float.intBitsToFloat(intBits);
		intArr.add( (double)intensity );
	...
	NativePeakList pList = new NativePeakList();
	pList.setMassArr( massArr.toNativeArray() );
	pList.setIntArr( intArr.toNativeArray() );
...
//    public static void decode(byte[] input, int precision)
    ...
    public static class NativePeakList 
    {

	double[] massArr;
	double[] intArr;

	public void setMassArr(double[] massArr)
	{
