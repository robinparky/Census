...
//, Hashtable<String,IndexedFile> ht
	...
	*/
		...
		for(int i=0;i<expSize;i++) {
			...
			if(conf.isLuciphorMode())
			{
				p.print("PREDICTED_SEQ1\t");
				p.print("PREDICTED_SEQ2\t");
				p.print("NUM_PPS\t");
				p.print("NUM_RPS\t");
				p.print("PSM_SCORE\t");
				p.print("DELTA_SCORE\t");
				p.print("PEP1_SCORE\t");
				p.print("PEP2_SCORE\t");
				p.print("GLOBAL_FLR_SCORE\t");
				p.print("LOCAL_FLR_SCORE\t");
	...
	//	    for(Iterator<String> fItr=fList.iterator(); fItr.hasNext(); ) {
			...
			boolean containsLuciphor = false;
	...
	//                                Still to generate the retention time for the specififc peptide.....
		...
		//
		//                    }
					Element luciphorElelment = pepEle.element("luciphor");
					LuciphorReader luciphorReader = new LuciphorReader();
					if(luciphorElelment!=null)
					{
						String id = peptide.getFileName() + "." + peptide.getScanNum() + "." + peptide.getScanNum() + "." + peptide.getChargeState();
						LuciphorReader.LuciphorPeptide luciphorPeptide = luciphorReader.readLuciphorElement(id, peptide.getSequence(),
								luciphorElelment);
						peptide.setLuciphorPeptide(luciphorPeptide);
					}
	...
	/*
		...
		*/
		...
		//System.out.println("====" + multiplePepSize + " " + mergedPeakArea + " " + mergedPeakArea/multiplePepSize + " " + multiplePepSize);
							...
							if(conf.isLuciphorMode())
							{
								if(pepArr[i].getLuciphorPeptide()!=null)
								{
									pepLineSb.append(pepArr[i].getLuciphorPeptide().predictedPep1).append("\t");
									pepLineSb.append(pepArr[i].getLuciphorPeptide().predictedPep2).append("\t");
									pepLineSb.append(pepArr[i].getLuciphorPeptide().numPPS).append("\t");
									pepLineSb.append(pepArr[i].getLuciphorPeptide().numRPS).append("\t");
									pepLineSb.append(pepArr[i].getLuciphorPeptide().psmScore).append("\t");
									pepLineSb.append(pepArr[i].getLuciphorPeptide().deltaScore).append("\t");
									pepLineSb.append(pepArr[i].getLuciphorPeptide().pep1score).append("\t");
									pepLineSb.append(pepArr[i].getLuciphorPeptide().pep2score).append("\t");
									pepLineSb.append(pepArr[i].getLuciphorPeptide().globalFLR).append("\t");
									pepLineSb.append(pepArr[i].getLuciphorPeptide().localFLR).append("\t");
								}
								else
								{
									for(int k=0;k<10;k++)
