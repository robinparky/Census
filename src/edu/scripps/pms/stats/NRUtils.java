...
/*  Now unscramble the permutation of the columns  */
    ...
    public static double LDA (double data_points[][], double projections[], double linear_coefficients[], int N_dim, int N_points, byte positive[]) {
...
//        accession = getDefline().substring( getDefline().indexOf('>')+1 );
        ...
        try
        {
            if( accession.startsWith("gi") && accession.contains("|") ) //NCBI
            {
                ...
                if( arr.length>=4 && ("sp".equals(arr[2]) || "ref".equals(arr[2]) || "gb".equals(arr[2])) )
                    accession = arr[3];
                else
                {
                    arr = accession.split(" ");
		...
		accession = arr[1];
	    ...
	    else if( accession.startsWith("Reverse_sp") ) //Reverse OMSSA format
            {
                String arr[] = accession.split("\\|");
            ...
            else if( accession.startsWith("Reverse_") || accession.startsWith("contam") || accession.startsWith("Contam")) //Reverse database
            {
		...
		int end;
                ...
                else  //cut by the first space or pipe
                    end = index;
                accession = accession.substring(0, end);
            }
        }
	...
	double[][] data_points = new double[2][5];
	...
	double projections[] = new double[5];
	projections[0] = 2;
	double linear_coefficients[] = new double[2];

	int N_dim = 2;
	int N_points = 5;
	byte[] positive = new byte[5];
	positive[0] = 0;
	positive[1] = 0;
	positive[2] = 0;
	positive[3] = 1;
	...
	//0 or 1.  2 is for unknown and not used for determinant analysis, but provides projections

	//LDA (double data_points[][], double projections[], double linear_coefficients[], int N_dim, int N_points, byte positive[]) {
	LDA (data_points, projections, linear_coefficients, N_dim, N_points, positive);
