...
//if(203 != (int)mass) continue;
			...
			//modify the code
                    ...
                    //if(end<0)
                     //   end = -end -2;
                    /*
                    if(start<0) //Cannot find index
                        start=-(++start); //Math.abs(++keyIndex);

                    if(start>=keys.length)
                        start--;

                    if(end<0) //Cannot find index
                        end=-(++end); //Math.abs(++keyIndex);

		    end--;  //subtract one more
...
//System.out.println("==-");
        ...
        else if(conf.getMsmsFragType() == conf.SPECIFIC_FRAGMENT_ION)
	{
		...
		if(null==result) return null;
                ...
                for(Iterator<ReportIon> mItr=massList.iterator(); mItr.hasNext(); )
                {
                    ...
                    sb.append( mItr.next().getMass() ).append(" ");
                }
    ...
    private static String buildDDResult(int peakStart, int peakEnd, int leftIndex, int rightIndex, IsoData[] intensityResult)
    {
        //harshil Shah

        StringBuffer sb = new StringBuffer();
        sb.append("P ").append((long)intensityResult[peakStart].getScanNumber()).append(" ").append((long)intensityResult[peakEnd].getScanNumber()).append(";");
//        for(int i=0;i<141;i++)
//        {
//            if(intensityResult[i] != null )
//            {
//                if(intensityResult[i].getScanNumber() == 3578)
//                System.out.println("");
//            }
//        }
        for(int i=leftIndex+1;i<rightIndex-1;i++)
	{
            ...
            if(intensityResult[i]==null)
                continue;
             sb.append(intensityResult[i].getScanNumber()+":");
//            for(int j=0;j<intensityResult[i].getLightData().length;j++)
            for(double d : intensityResult[i].getLightData())
                sb.append(d).append(" ");
            sb.append(":");
            for(double d : intensityResult[i].getHeavyData())
                sb.append(d).append(" ");
            sb.append(";");
...
//compare thise
	...
	double[][] result = new double[2][massArr.length];
	result[0] = massArr;
	result[1] = intArr;
...
//		    moveRightKeyIndex += numIsoWindow;
    ...
    int leftIndex=maxWindow+margin;
        ...
        boolean isHighIntensity=true;  //if the intensity is lower than one third of average intensity, this becomes false
        double[][] arr = new double[steepArea][3];
        IsoData[] tempIntArr = new IsoData[steepArea];

        double[][] prevArr = new double[steepArea][3];
        IsoData[] prevIntArr = new IsoData[steepArea];
        if(leftIndex+steepArea>=0)
        {
            //for(int i=0;i<300;i++)
	        while(true)
		...
		if(isHighIntensity && ((double)totalIntensity/(rightIndex-leftIndex-1)/1.5>area2/3))
		    isHighIntensity = false;
                ...
                if( (isGoingUp && isHighIntensity) || areaRatio<steepRatioThreshold )
                {
                    ...
                    result[rightIndex] = arr[0];
                    intensityResult[rightIndex] = tempIntArr[0];
			...
			if(result.length<=rightIndex)
			    break;
                        ...
                        result[rightIndex] = arr[k];
                        intensityResult[rightIndex] = tempIntArr[k];
                        //System.out.println(arr[0][1] + " " + arr[1][1] + " " + arr[2][1] + " " + result[rightIndex-1][1]);
...
*/
 ...
 //       sModel.setYioRef(yionRef);
        ...
        throws IOException, CensusIndexOutOfBoundException, Exception
    {
        ...
        sModel.setRef2IsoArr(refIsoArr);

        return peakFinding3Plex(sModel, range, keyIndex);
    }
    public static String calculateFullMS(
            int keyIndex,
            IndexedFile iFile,
    ...
    public static void getSpectrumArr(int[] keys, int curIndex, TIntLongHashMap index, Object ofile)
            throws IOException, CensusIndexOutOfBoundException, Exception
    {
	RandomAccessFile file = (RandomAccessFile)ofile;
	...
	int spaceCount=0;
		...
		switch(ch)
		{
			...
			spaceCount=0;
                        ...
                        //try {
                       // if(intensity.toString().equals(0-10703687647232363302")){
                       //     System.out.println("");
                      //  }
                        ...
                        try {
                            ...
                            System.out.println("wrong file format:\t" + mass.toString());
                            System.out.println("wrong file format:\t" + intensity.toString());
                        }
...
*/
	...
	else if(reader instanceof MzxmlSpectrumReader)
	{
	    ...
	    readFullSpectrumMzXml(keys, curIndex, mzReader);
         ...
         isoData.setLightData(getIntensity(massArr, intArr, samIsoArr));
        if(null != refIsoArr) {
            isoData.setHeavyData(getIntensity(massArr, intArr, refIsoArr));
	}

              isoData.setScanNumber(keys[curIndex]);
	return isoData;
    }
    //High resolution //lalebling data
...
//-----------
	double[] tempArr = intensitySum(massArr, intArr, samIsoArr);
//harshil SHah----- create a method that will call and return the intensity arr.....
        result[1] = tempArr[0];     //intensity
...
//added by Harshil Shah
        public static String massWrite(double[] samIsoArr,double[] refIsoArr)
        {
            //samIsoArr is lihght and refIsoArr is heavy.........
            StringBuffer sb = new StringBuffer();
        ...
        public static String massWrite(double[] samIsoArr,double[] refIsoArr, double[] refIsoArr2)
        {
          //samIsoArr is lihght, refIsoArr is medium, and refIsoArr2 is heavy.........
          StringBuffer sb = new StringBuffer();
          for(double value: samIsoArr)
          {
            sb.append(value +",");
        ...
        public static String intensityWrite(double[] samIsoIntArr,double[] refIsoIntArr, double[] ref2IsoIntArr) {

          //samIsoArr is lihght and refIsoArr is heavy.........
          StringBuffer sb = new StringBuffer();
          for(int i =0;i<samIsoIntArr.length;i++)
          {
            sb.append(samIsoIntArr[i] +",");
          }
          sb.append(":");
          for(int i =0;i<refIsoIntArr.length;i++)
          {
            sb.append(refIsoIntArr[i] +",");
          }
          sb.append(":");
          for(int i =0;i<ref2IsoIntArr.length;i++)
          {
            sb.append(ref2IsoIntArr[i] +",");
          }
          return sb.toString();
        }

        public static String intensityWrite(double[] samIsoIntArr,double[] refIsoIntArr, int lightLength,int heavyLength)
        {
            //samIsoArr is lihght and refIsoArr is heavy.........
            StringBuffer sb = new StringBuffer();
